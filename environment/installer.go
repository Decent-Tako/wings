...
// the live output callback has consumed the runtime stream to EOF. If runtime
// artifacts exist when an installer fails, Execute returns their id with the
// error so the caller can copy logs before removal.
